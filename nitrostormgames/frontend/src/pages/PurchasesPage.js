      ...
      <section id="purchases" className="tw-pb-6">
        <div className="tw-container tw-mt-16 tw-flex tw-gap-6 tw-flex-col">
          {purchases.map((purchase) => {
            return (
              <div className="tw-flex tw-flex-col tw-ml-6 tw-bg-bookmark-white tw-rounded-md tw-w-full tw-min-h-fit" key={JSON.stringify(purchase)}>
                <div className="tw-m-4">
                  ...
                  <div className="tw-ml-6 tw-flex tw-flex-col tw-gap-2">
                    {purchase.content.map((game) => {
                      return (
                      <a key={game.name} href={game.download_url} className="hover:tw-underline tw-text-bookmark-red">Play {game.name}</a>
                      )
                    })}
                  </div>
                </div>
